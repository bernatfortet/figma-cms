"use strict";
var __awaiter = (this && this.__awaiter) || function (thisArg, _arguments, P, generator) {
    return new (P || (P = Promise))(function (resolve, reject) {
        function fulfilled(value) { try { step(generator.next(value)); } catch (e) { reject(e); } }
        function rejected(value) { try { step(generator["throw"](value)); } catch (e) { reject(e); } }
        function step(result) { result.done ? resolve(result.value) : new P(function (resolve) { resolve(result.value); }).then(fulfilled, rejected); }
        step((generator = generator.apply(thisArg, _arguments || [])).next());
    });
};
var __generator = (this && this.__generator) || function (thisArg, body) {
    var _ = { label: 0, sent: function() { if (t[0] & 1) throw t[1]; return t[1]; }, trys: [], ops: [] }, f, y, t, g;
    return g = { next: verb(0), "throw": verb(1), "return": verb(2) }, typeof Symbol === "function" && (g[Symbol.iterator] = function() { return this; }), g;
    function verb(n) { return function (v) { return step([n, v]); }; }
    function step(op) {
        if (f) throw new TypeError("Generator is already executing.");
        while (_) try {
            if (f = 1, y && (t = op[0] & 2 ? y["return"] : op[0] ? y["throw"] || ((t = y["return"]) && t.call(y), 0) : y.next) && !(t = t.call(y, op[1])).done) return t;
            if (y = 0, t) op = [op[0] & 2, t.value];
            switch (op[0]) {
                case 0: case 1: t = op; break;
                case 4: _.label++; return { value: op[1], done: false };
                case 5: _.label++; y = op[1]; op = [0]; continue;
                case 7: op = _.ops.pop(); _.trys.pop(); continue;
                default:
                    if (!(t = _.trys, t = t.length > 0 && t[t.length - 1]) && (op[0] === 6 || op[0] === 2)) { _ = 0; continue; }
                    if (op[0] === 3 && (!t || (op[1] > t[0] && op[1] < t[3]))) { _.label = op[1]; break; }
                    if (op[0] === 6 && _.label < t[1]) { _.label = t[1]; t = op; break; }
                    if (t && _.label < t[2]) { _.label = t[2]; _.ops.push(op); break; }
                    if (t[2]) _.ops.pop();
                    _.trys.pop(); continue;
            }
            op = body.call(thisArg, _);
        } catch (e) { op = [6, e]; y = 0; } finally { f = t = 0; }
        if (op[0] & 5) throw op[1]; return { value: op[0] ? op[1] : void 0, done: true };
    }
};
var _this = this;
Object.defineProperty(exports, "__esModule", { value: true });
var fs = require("fs-extra");
var request = require("request-promise");
var sharp = require("sharp");
var figma = require("./figma");
var figma_db_1 = require("./figma_db");
var OUTPUT_PATH = '../public/images/';
var init = function () { return __awaiter(_this, void 0, void 0, function () {
    var images;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0: return [4 /*yield*/, figma.fetchFileImages(figma_db_1.FILE_KEY)];
            case 1:
                images = _a.sent();
                return [4 /*yield*/, exports.downloadImages(images)];
            case 2:
                _a.sent();
                return [2 /*return*/];
        }
    });
}); };
exports.downloadImages = function (images) { return __awaiter(_this, void 0, void 0, function () {
    var imagePromises, id, url;
    return __generator(this, function (_a) {
        switch (_a.label) {
            case 0:
                checkImagesFolderAndCreate();
                return [4 /*yield*/, getMissingImages(images)];
            case 1:
                images = _a.sent();
                console.log(Object.keys(images).length + " Images are missing");
                imagePromises = [];
                for (id in images) {
                    url = images[id];
                    imagePromises.push(getImageObjectFromUrl(url, id));
                }
                return [2 /*return*/, Promise.all(imagePromises)];
        }
    });
}); };
function getMissingImages(images) {
    return __awaiter(this, void 0, void 0, function () {
        var missingImages, id, url, exists;
        return __generator(this, function (_a) {
            console.log("\uD83D\uDD32  Checking for missing images");
            missingImages = {};
            for (id in images) {
                url = images[id];
                exists = fs.pathExistsSync("" + OUTPUT_PATH + id + ".jpg");
                if (!exists) {
                    console.error("Image for " + id + " is missing: ");
                    missingImages[id] = url;
                }
            }
            return [2 /*return*/, missingImages];
        });
    });
}
function getImageObjectFromUrl(url, fileName) {
    var _this = this;
    var COVER_AR = 2.5;
    return request.get({ url: url, encoding: null })
        .then(function (res) { return __awaiter(_this, void 0, void 0, function () {
        var Sharp, _a, width, height, aspectRatio, size;
        return __generator(this, function (_b) {
            switch (_b.label) {
                case 0:
                    console.log("Getting Image " + fileName);
                    return [4 /*yield*/, sharp(res)];
                case 1:
                    Sharp = _b.sent();
                    return [4 /*yield*/, Sharp.metadata()];
                case 2:
                    _a = _b.sent(), width = _a.width, height = _a.height;
                    if (fileName == 'b51210344b8062c84cace263fd80ae027cbcf05f') {
                        console.log("width", width);
                        console.log("height", height);
                    }
                    aspectRatio = width / height;
                    size = aspectRatio >= COVER_AR ? [1500, 300] : [600];
                    Sharp.resize(size[0], size[1]).jpeg({ quality: 90 }).toFile("../public/images/" + fileName + ".jpg");
                    return [2 /*return*/];
            }
        });
    }); })
        .catch(function (error) { return null; });
}
init();
function checkImagesFolderAndCreate() {
    if (!fs.existsSync(OUTPUT_PATH))
        fs.mkdirSync(OUTPUT_PATH);
}
